...
type AlertStore = ReturnType<typeof createSharedStore<AlertStoreState, typeof EVENTS>>;
...
function getStore(): AlertStore {
  if (!store) {
    store = createSharedStore<AlertStoreState, typeof EVENTS>({
      ...
      events: EVENTS,
      onEventChange: (state, event, payload: Alert) => {
        switch (event) {
          case "ADDED":
          case "MODIFIED":
            return { alerts: { ...state.alerts, [payload.id]: payload }, loading: false };
          case "DELETED": {
            const { [payload.id]: _, ...alerts } = state.alerts;
    ...
    Promise.all(
      ...
      for (const alerts of results) {
        for (const alert of alerts) {
          s.updateState("ADDED", alert);
        }
      }
    });

    // Subscribe to alerts WS topic if server supports it
    const unsub = api.fleetshift.on("alerts", (event: { verb: string; object: Alert }) => {
